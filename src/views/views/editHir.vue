<template>
  <div class="editTwc">
    ...
    <div class="button_commit" style="text-align: left;padding-left: 15px;padding-top: 10px;">
      ...
      <router-link v-if="this.type==='oc'" :to="{name:'techdSection3',query:{id:this.id,type:'oc',type_name:type_name}}">
  ...
  import {getSign} from '@/utils/md5'
    export default {
      ...
      mounted:function(){
        ...
        this.rat_hir.id = this.id; // 修改rat_twc id
        // 初始化加载hir entry information
        this.findHirByid();
      },
        data(){
          return{
      ...
      methods:{
        ...
        findHirByid(){
          ...
          // this.file_name === '3' 查询 hir copy
          console.log("findHirByid() -> type_name = " +  this.type_name);
          if (this.type_name === '3'){
        ...
        editEntry(){
          // 接着编辑hir
          let url = requestPath() + "EditHirEntry";
          // edit hir temp (hir copy)
          if (this.type_name === '3'){
            url = requestPath()+"EditHirTempEntry";
          }
          console.log("edit hir url->" + url);

          console.log("edit hir file_name->"+this.file_name);
          // 打印一下remark info
          ...
          if (this.type === 'oc'){
            ...
            // 執行者
            this.rat_hir.creator = window.localStorage.getItem("user_name");
            // send email
            this.rat_hir.send_email = window.localStorage.getItem("user_email");
          }
