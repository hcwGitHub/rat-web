...
<!--                <el-input v-model="rat_contact_details.project_name" placeholder=""></el-input>-->
    ...
    <div class="button_commit" style="text-align: left;padding-left: 15px;padding-top: 10px;">
      <router-link v-if="this.type==='oc'" :to="{name:'techdSection4',query:{id:this.id,type:'oc'}}">
      ...
      <router-link v-if="this.type==='mobility'" :to="{name:'viewContactDetail',query:{id:this.id,type:'mobility',identifier:identifier}}">
        <el-button @click="submitEntry" style="background-color: #1c68a4;color: white"><i class="el-icon-check" style="padding-right:5px; "></i>Save</el-button>
      </router-link>
  ...
  import {getSign} from '@/utils/md5'
    export default {
        ...
        mounted:function(){
          ...
          this.rat_contact_details.id = this.id; // 修改rat_contact_details id
      ...
      methods:{
          ...
          findContactDetailsByid(){
            ...
            this.$axios.get(url,
              ...
              if (response.data.result === "SUCCESS") {
                ...
                if (this.rat_contact_details.capacity !== '') {
                  this.checkList = this.rat_contact_details.capacity.split(',');
                }
        ...
        // upload files and edit contact details entry information
        submitEntry(){
        ...
        editEntry(){
          // 接着编辑contact details
          let url = requestPath() + "EditContactDetailsEntry";
          console.log("edit url->" + url);
          this.rat_contact_details.attachments = this.file_name;
          console.log("edit contact details file_name->"+this.file_name);
          ...
          if (this.type === 'oc'){
            // 執行者
            this.rat_contact_details.creator = window.localStorage.getItem("oc_user_name");
            // send email
            this.rat_contact_details.send_email = window.localStorage.getItem("oc_user_email");
          }else {
            // 執行者
            this.rat_contact_details.creator = window.localStorage.getItem("user_name");
            // send email
            this.rat_contact_details.send_email = window.localStorage.getItem("user_email");
          }
          ...
          //
          // // send email
          // this.rat_contact_details.send_email = window.localStorage.getItem("creator_email");

          // 打印一下remark info
          // console.log("rat_contact_details->" + this.rat_contact_details.remark);

          this.rat_contact_details.oc_mobility_type = this.type;
          // 21/07/2021 修復郵件鏈接
